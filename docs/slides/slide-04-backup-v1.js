...
function createSlide(pres, theme) {
  ...
  const toolComponents = [
    { name: "MCP协议", desc: "标准" },
    { name: "Skill库", desc: "技能" },
    { name: "Function", desc: "函数" },
    { name: "插件", desc: "扩展" }
  ];
  toolComponents.forEach((comp, idx) => {
    const row = Math.floor(idx / 2);
    const col = idx % 2;
    const xPos = 0.65 + col * 1.5;
    const yPos = 4.03 + row * 0.24;
    slide.addShape(pres.shapes.ROUNDED_RECTANGLE, {
      x: xPos, y: yPos, w: 1.4, h: 0.22,
      fill: { color: "FFFFFF" },
      line: { color: "9C27B0", width: 0.5 },
      rectRadius: 0.04
    });
    slide.addText(comp.name, {
      x: xPos + 0.03, y: yPos, w: 0.75, h: 0.22,
      fontSize: 7, fontFace: "Microsoft YaHei",
      color: "9C27B0", bold: true,
      valign: "middle"
    });
    slide.addText(comp.desc, {
      x: xPos + 0.75, y: yPos, w: 0.62, h: 0.22,
      fontSize: 6, fontFace: "Microsoft YaHei",
      color: "607D8B",
      valign: "middle"
    });
