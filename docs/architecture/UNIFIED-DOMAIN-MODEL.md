...
当前模型需要同时覆盖企业级 Agent 工作台场景。除下文已有 DevOps 对象外，后续 4+1 讨论必须把以下作用域对象作为前置模型：

| 对象 | 定位 |
|------|------|
| Tenant / Organization | 企业边界，承载统一身份、策略、审计和数据隔离 |
| Workspace | 协作边界，可以对应业务域、部门、项目群或平台团队 |
| Project | 主要隔离单元，承载知识、任务、Agent、连接器和运行历史 |
| User / Team / Role | 身份和授权主体，对接企业 IAM 后映射为内部统一模型 |
| Agent | 执行主体，可以是平台级、工作区级或项目级，但运行时必须绑定用户和项目上下文 |
| Session | 用户和 Agent 的交互上下文，不等同于审计事实源 |
| Run / Plan / Step | 一次任务运行、计划和阶段，是执行、回放和审计的主线 |
| Tool / Connector | 对内部平台和外部系统的受控访问边界 |
| Knowledge Collection | 项目知识、组织知识、运行证据、索引和权限元数据集合 |
| Memory / Skill | 长期记忆和可复用操作经验，和普通文档知识分开治理 |
| Approval / Policy | 高风险动作的策略判断、人工确认和例外记录 |
| Event / Artifact | append-only 事实事件和任务产物，用于审计、投影和追溯 |

核心原则：

- `Project` 是默认数据隔离和权限评估单元。
- `Session` 服务交互体验，`Run / Event` 才是运行事实层。
- `Knowledge Collection` 必须保存来源、版本、时间、项目和 ACL。
- `Agent` 不能脱离用户和项目权限独立行动。
- 外部系统仍是 source of truth，AgentCenter 保存映射、证据、索引、摘要和关系。
