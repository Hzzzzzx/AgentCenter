...
Composer 的输入边界必须固定在产品运行态数据内：`workflow_definition` / `workflow_node_definition`、工作项、会话消息、产物、交互历史，以及项目级 Runtime Skill catalog。服务启动和工作流运行不得读取、依赖或要求 `.sisyphus/` 存在；`.sisyphus/` 也不得作为编排顺序、Skill 选择、推荐 Skill、Prompt 片段或节点执行依据。`.sisyphus/` 只保存开发计划、验证证据和 handoff。
