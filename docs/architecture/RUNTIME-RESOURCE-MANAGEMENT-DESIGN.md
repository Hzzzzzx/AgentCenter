...
- M1 不强求 OpenCode 热加载；如果 runtime 不支持热加载，先标记资源已刷新，让后续会话消息重建 Runtime session，而不是立即重启共享 `opencode serve`。
...
2. 如果只支持重新扫描，调用扫描接口。
3. 如果都不支持，标记当前项目 Runtime 资源已刷新，让后续会话消息重建 OpenCode session；不在活跃对话中立即重启共享 `opencode serve`。

重启策略：

- 已有会话继续使用当前 Runtime 连接，避免中断正在生成的回复。
- 会话资源状态通过 `reloadRequired` 提示“下次消息重载”。
- 下一次消息进入 Bridge 时重新 `ensureSession`，必要时创建新的 OpenCode session。
- 新建会话必须使用最新 Skill/MCP。
- 正在执行的工作流节点不强行中断；刷新完成后从下一个节点开始使用新资源。
- 只有运维手动重启或未来 Runtime 支持安全 drain 时，才重启共享 `opencode serve`。
...
- 资源刷新不会中断正在生成的会话；当前会话下次发送消息时重建 Runtime session。
- 如果 OpenCode serve 被运维手动重启，前端 SSE 能恢复连接并刷新资源状态。
